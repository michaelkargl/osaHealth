...
.PARAMETER VaultBaseUri
    Base URI of the Vault server (e.g. http://localhost:8200).

.PARAMETER DaprSecretsPath
    File path where the DAPR secrets JSON will be written.

.PARAMETER MongoUser
    MongoDB username to seed into Vault on first run.

.PARAMETER MongoPassword
    MongoDB password to seed into Vault on first run.

.PARAMETER UnsealKey
    Vault unseal key; required on every run after first initialization.

.PARAMETER RootToken
    Vault root token; used to authenticate API calls after first initialization.

.PARAMETER PollIntervalSec
    Seconds between Vault reachability polls. Defaults to 2.
#>
...
.EXAMPLE
    Invoke-VaultApi -VaultBaseUri 'http://localhost:8200' -Path 'v1/sys/seal-status' -Method Get

.EXAMPLE
    Invoke-VaultApi -VaultBaseUri 'http://localhost:8200' -Path 'v1/sys/init' -Method Put -Body @{ secret_shares = 1; secret_threshold = 1 }

.EXAMPLE
    Invoke-VaultApi -VaultBaseUri 'http://localhost:8200' -Path 'v1/sys/unseal' -Method Put -Body @{ key = 'unseal-key-here' } -Token $rootToken
#>
function Invoke-VaultApi {
    ...
    $uri = '{0}/{1}' -f $VaultBaseUri.TrimEnd('/'), $Path.TrimStart('/')
...
function Test-VaultRunning {
    ...
    try {
        Invoke-VaultApi -VaultBaseUri $VaultBaseUri -Path 'v1/sys/seal-status' -Method Get | Out-Null
        return $true
    }
    catch {
        return $false
...
function Test-VaultInitialized {
    ...
    try {
        $status = Invoke-VaultApi -VaultBaseUri $VaultBaseUri -Path 'v1/sys/init' -Method Get
...
function Wait-ForVault {
    ...
    if ($WhatIfPreference) {
        Write-Host "What if: Waiting for Vault at $VaultBaseUri to become reachable."
        return
    }

    while (-not (Test-VaultRunning -VaultBaseUri $VaultBaseUri)) {
...
.EXAMPLE
    $init = Initialize-Vault -VaultBaseUri 'http://localhost:8200'
    $init.UnsealKey
    $init.RootToken
#>
function Initialize-Vault {
    ...
    if (-not $PSCmdlet.ShouldProcess($VaultBaseUri, 'Initialize Vault (operator init, 1 key share)')) {
    ...
    $response = Invoke-VaultApi -VaultBaseUri $VaultBaseUri -Path 'v1/sys/init' -Method Put -Body $body
...
function Invoke-VaultUnseal {
    ...
    if ($PSCmdlet.ShouldProcess($VaultBaseUri, 'Unseal Vault')) {
        $response = Invoke-VaultApi -VaultBaseUri $VaultBaseUri -Path 'v1/sys/unseal' -Method Put -Body @{ key = $Key }
        if ($response.sealed) {
...
function Enable-SecretsEngine {
    ...
    if ($PSCmdlet.ShouldProcess("$VaultBaseUri (secret)", 'Enable kv-v2 secrets engine')) {
        $body = @{ type = 'kv'; options = @{ version = '2' } }
        Invoke-VaultApi -VaultBaseUri $VaultBaseUri -Path "v1/sys/mounts/secret" -Method Post -Body $body -Token $Token | Out-Null
    }
}
...
.EXAMPLE
    Set-VaultSecret -VaultBaseUri 'http://localhost:8200' -Token $root -Key 'mongodb' -Value @{ username = 'admin'; password = 's3cr3t' }
#>
function Set-VaultSecret {
    ...
    if ($PSCmdlet.ShouldProcess("$VaultBaseUri (secret/$Key)", 'Write vault secret')) {
        $body = @{ data = $Value }
        Invoke-VaultApi -VaultBaseUri $VaultBaseUri -Path "v1/secret/data/$Key" -Method Post -Body $body -Token $Token | Out-Null
    }
}
...
function Invoke-Main {
    ...
    Wait-ForVault -VaultBaseUri $VaultBaseUri -PollIntervalSec $PollIntervalSec

    if (-not (Test-VaultInitialized -VaultBaseUri $VaultBaseUri)) {
        Write-Host '=== First run: initializing Vault ==='
        $init = Initialize-Vault -VaultBaseUri $VaultBaseUri
        Invoke-VaultUnseal -VaultBaseUri $VaultBaseUri -Key $init.UnsealKey
        Enable-SecretsEngine -VaultBaseUri $VaultBaseUri -Token $init.RootToken
        Set-VaultSecret -VaultBaseUri $VaultBaseUri -Token $init.RootToken -Key 'mongodb' -Value @{ username = $MongoUser; password = $MongoPassword }
