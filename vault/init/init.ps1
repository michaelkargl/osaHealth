...
.PARAMETER UnsealKey
    Vault unseal key. Empty on first run (generated by Vault init); required on every subsequent run.

.PARAMETER RootToken
    Vault root token. Empty on first run (generated by Vault init); required on every subsequent run.
