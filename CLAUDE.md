...
| Action           | Command                                                                             |
|------------------|-------------------------------------------------------------------------------------|
| Create task      | `npx backlog task create "Title"`                                                       |
| With description | `npx backlog task create "Title" -d "Description"`                                      |
| With AC          | `npx backlog task create "Title" --ac "Criterion 1" --ac "Criterion 2"`                 |
| With final summary | `npx backlog task create "Title" --final-summary "PR-style summary"`                 |
| With references  | `npx backlog task create "Title" --ref src/api.ts --ref https://github.com/issue/123`   |
| With documentation | `npx backlog task create "Title" --doc https://design-docs.example.com`               |
| With modified files | `npx backlog task create "Title" --modified-file src/api.ts --modified-file src/ui.ts` |
| With all options | `npx backlog task create "Title" -d "Desc" -a @sara -s "To Do" -l auth --priority high --ref src/api.ts --doc docs/spec.md --modified-file src/api.ts` |
| Create draft     | `npx backlog task create "Title" --draft`                                               |
| Create subtask   | `npx backlog task create "Title" -p 42`                                                 |

### Task Modification

| Action           | Command                                     |
|------------------|---------------------------------------------|
| Edit title       | `npx backlog task edit 42 -t "New Title"`       |
| Edit description | `npx backlog task edit 42 -d "New description"` |
| Change status    | `npx backlog task edit 42 -s "In Progress"`     |
| Assign           | `npx backlog task edit 42 -a @sara`             |
| Add labels       | `npx backlog task edit 42 -l backend,api`       |
| Set priority     | `npx backlog task edit 42 --priority high`      |

### Acceptance Criteria Management

| Action              | Command                                                                     |
|---------------------|-----------------------------------------------------------------------------|
| Add AC              | `npx backlog task edit 42 --ac "New criterion" --ac "Another"`                  |
| Remove AC #2        | `npx backlog task edit 42 --remove-ac 2`                                        |
| Remove multiple ACs | `npx backlog task edit 42 --remove-ac 2 --remove-ac 4`                          |
| Check AC #1         | `npx backlog task edit 42 --check-ac 1`                                         |
| Check multiple ACs  | `npx backlog task edit 42 --check-ac 1 --check-ac 3`                            |
| Uncheck AC #3       | `npx backlog task edit 42 --uncheck-ac 3`                                       |
| Mixed operations    | `npx backlog task edit 42 --check-ac 1 --uncheck-ac 2 --remove-ac 3 --ac "New"` |
...
```bash
npx backlog task edit 42 --notes "First line"
npx backlog task edit 42 --append-notes "Second line"
npx backlog task edit 42 --append-notes "Third line"
```
...
- Bash/Zsh (ANSI‑C quoting):
  ...
  ```bash
  npx backlog task edit 42 --notes $'Line1\nLine2'
  ```

- POSIX sh (command substitution + printf):

  ```bash
  npx backlog task edit 42 --notes "$(printf 'Line1\nLine2')"
  ```

- PowerShell (backtick‑n):

  ```powershell
  npx backlog task edit 42 --notes "Line1`nLine2"
  ```
...
- When using CLI flags like `--append-notes`, remember to include explicit
  ...
  ```bash
  npx backlog task edit 42 --append-notes "- Added new API endpoint" \
...
npx backlog doc create "API Guidelines"

# Create in a subdirectory (nested paths supported)
npx backlog doc create "Setup Guide" -p guides/setup

# Specify type at creation time
npx backlog doc create "Architecture" -t guide

# Update content while preserving omitted metadata
npx backlog doc update doc-1 --content "Updated markdown"

# Update metadata or move a doc within backlog/docs/
npx backlog doc update doc-1 --title "Setup Handbook" -t guide --tags setup,runbook -p guides

# List all docs (searched globally across subdirectories)
npx backlog doc list

# View a specific doc
npx backlog doc view doc-1
```
...
| Action             | Command                                      |
|--------------------|----------------------------------------------|
| View task          | `npx backlog task 42 --plain`                    |
| List tasks         | `npx backlog task list --plain`                  |
| Search tasks       | `npx backlog search "topic" --plain`              |
| Search with filter | `npx backlog search "api" --status "To Do" --plain` |
| Search by modified file | `npx backlog search --modified-file src/api.ts --plain` |
| Filter by status   | `npx backlog task list -s "In Progress" --plain` |
| Filter by assignee | `npx backlog task list -a @sara --plain`         |
| Archive task       | `npx backlog task archive 42`                    |
| Demote to draft    | `npx backlog task demote 42`                     |
